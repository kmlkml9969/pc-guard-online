...
if ($WithWeixin) {
  Require-Command node "Install Node.js 22+ first, or add it to PATH."
  Require-Command npm "Install Node.js/npm first, or add it to PATH."
}
...
@echo off
cd /d "%~dp0"
"%APPDATA%\npm\openclaw.cmd" gateway --force >> "%~dp0openclaw_gateway.out.log" 2>> "%~dp0openclaw_gateway.err.log"
'@
...
if ($WithWeixin) {
  Write-Output "Installing OpenClaw Weixin plugin. This can take a few minutes..."
  npx -y @tencent-weixin/openclaw-weixin-cli@latest install

  & (Join-Path $InstallDir "patch_openclaw_weixin_pcguard.ps1") -PcGuardConfigPath $serverConfigPath
  ...
  Start-Process -FilePath (Join-Path $env:APPDATA "npm\openclaw.cmd") -ArgumentList @("gateway", "--force") -WorkingDirectory $InstallDir -WindowStyle Hidden
}
