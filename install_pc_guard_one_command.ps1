...
if ($WithWeixin) {
  ...
  Stop-OpenClawGateway
  Start-Process -FilePath "python" -ArgumentList @("`"$InstallDir\pc_guard_weixin_direct.py`"", "--pc-config", "`"$serverConfigPath`"") -WorkingDirectory $InstallDir -WindowStyle Hidden
